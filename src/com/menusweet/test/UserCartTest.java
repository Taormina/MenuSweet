...
public class UserCartTest extends TestCase {
    ...
    public UserCartTest(String name) {
        super(name);
        itemSet.add(chickenCurry);
        itemSet.add(beefCurry);
        itemSet.add(tofuCurry);
        itemSet.add(softDrink);
        itemSet.add(water);
    }
    ...
    public void testAddItem() {
        ...
        UserCart cart = new UserCart(0.07, itemSet);
        assertTrue("The cart is not properly initialized.", cart.isEmpty());

        cart.testAddItem(0, 1);
        cost = chickenCurry.getPrice();
        assertFalse("The cart is empty after an item has been added", cart.isEmpty());
        assertEquals("The subtotal was not updated correctly when 1 new item was added.", cost, cart.getTotalPrice());

        cart.testAddItem(1, 2);
        cost += 2 * beefCurry.getPrice();
        assertEquals("The subtotal was not updated correctly when 2 new items were added.", cost, cart.getTotalPrice());

        cart.testAddItem(3, 3);
        cost += 3 * softDrink.getPrice();
        assertEquals("The subtotal was not updated correctly when 3 new items were added.", cost, cart.getTotalPrice());

        caughtException = false;
        try {
            cart.testAddItem(-1, 3);
        }  catch (IndexOutOfBoundsException e) {
            caughtException = true;
        }
        assertTrue("Did not throw the correct exception when given a null value.", caughtException);
        assertEquals("The subtotal was updated when a null parameter is passed to testAddItem.", cost, cart.getTotalPrice());
        ...
        assertTrue("Did not throw the correct exception when given a null value.", caughtException);
        assertEquals("The subtotal was updated when an invalid parameter is passed to testAddItem.", cost, cart.getTotalPrice());
        ...
        assertEquals("The subtotal was updated when multiple invalid parameters are passed to testAddItem.", cost, cart.getTotalPrice());
    ...
    public void testRemoveItem() {
        ...
        cart.testAddItem(0, 1);  // 0
        cart.testAddItem(1, 2);     // 1
        cart.testAddItem(3, 3);     // 2
        cost = chickenCurry.getPrice() + 2 * beefCurry.getPrice() + 3 * softDrink.getPrice();
        assertEquals("The cart was not properly prepped for testing.", cart.getTotalPrice(), cost);

        cart.testRemoveItem(3); // softDrink
        cart.testAddItem(4, 3);         // 3
